...
def convert_heic_to_jpg(heic_dir, output_quality=50, max_workers=4) -> None:
    ...
    #### Args:
        ...
        - output_quality (int, optional): Quality of the output JPG images
    ...
    if os.path.exists(jpg_dir):
        user_input = input("Existing \n'ConvertedFiles' folder detected.\n"
                           "Delete and proceed? (y/n): ")
    ...
    heic_files = [file for file in os.listdir(heic_dir)
    ...
    for file_name in heic_files:
        ...
        jpg_path = os.path.join(
            jpg_dir, os.path.splitext(file_name)[0] + ".jpg")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(convert_single_file, heic_path, jpg_path,
                            output_quality): heic_path
        ...
        for future in as_completed(future_to_file):
            ...
            try:
                ...
                print(f"Conversion progress: {progress}%", end="\r",
                      flush=True)
            except Exception as e:
                logging.error("Error occurred during conversion of '%s': %s",
                              heic_file, e)

    print(f"\nConversion completed successfully. {num_converted}"
          f" files converted.")
